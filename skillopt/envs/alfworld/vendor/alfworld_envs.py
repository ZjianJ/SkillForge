...
def _build_base_env(config, is_train, eval_dataset, gamefile=None):
    ...
    class SingleGameEnv(env_cls):
        def collect_game_files(self, verbose=False):
            self.game_files = [gamefile]
            self.num_games = 1

    return SingleGameEnv(config, train_eval=train_eval)
...
def _worker_loop(cmd_q, result_q, config, seed, is_train, eval_dataset, gamefile):
    ...
    try:
        base_env = _build_base_env(config, is_train, eval_dataset, gamefile)
