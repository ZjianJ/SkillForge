...
from skillopt.utils.paths import safe_prediction_dir_name
import random
import traceback
...
def fmt_minibatch_trajectories(
    ...
    for idx, item in enumerate(items, 1):
        tid = str(item["id"])
        pred_tid = safe_prediction_dir_name(tid)
        conv_path = os.path.join(prediction_dir, pred_tid, "conversation.json")
        ...
        if not target_prompt:
            prompt_path = os.path.join(prediction_dir, pred_tid, "target_system_prompt.txt")
        ...
        if not user_prompt:
            user_prompt_path = os.path.join(prediction_dir, pred_tid, "target_user_prompt.txt")
        ...
        if os.environ.get("REFLACT_CODEX_TRACE_TO_OPTIMIZER", "0") == "1":
            ...
            if not codex_trace_summary:
                codex_trace_summary_path = os.path.join(prediction_dir, pred_tid, "codex_trace_summary.txt")
        ...
        if not preview:
            preview_path = os.path.join(prediction_dir, pred_tid, "spreadsheet_preview.txt")
